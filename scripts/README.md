...
This directory contains scripts for database operations in the Forex Trading AI System.

## Database Initialization and Migration

The system uses a PostgreSQL database with a versioned migration system. The following scripts are available:

### run_migrations.sh

A shell script to run database migrations. It has three modes:

- **init**: Initialize a new database with the base schema
...
The Python script that handles database migrations. It's usually called through run_migrations.sh, but can be used directly:

```bash
# Initialize a new database
python scripts/db_migrations.py init

# Run pending migrations
python scripts/db_migrations.py migrate

# Create a new migration
python scripts/db_migrations.py create add_new_table
```

## Database Backup and Restore

The system provides scripts for backing up and restoring the database:

### backup_db.sh

A shell script to create a compressed backup of the database:
...
The script creates a timestamped backup file in the `backups/db` directory.

### restore_db.sh

A shell script to restore a database from a backup:
...
**Warning**: This will drop and recreate the database, so all current data will be lost.

## Database Schema

The initial database schema is defined in the `db_init` directory:

- **01_create_tables.sql**: Creates the database tables, indexes, and constraints
- **02_seed_data.sql**: Adds initial seed data for testing
...
- **DB_NAME**: Database name (default: forex)
- **DB_USER**: Database username (default: postgres)
- **DB_PASSWORD**: Database password (default: postgres)

These should be defined in the .env file in the project root.
